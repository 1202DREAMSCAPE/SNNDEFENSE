...
# --- Define helper ---
def compute_threshold_f1(distances, labels, num_thresholds=1000, visualize=False, save_path=None):
    """
    ...
    # determine the minimum and maximum distance values to define the sweep range
    dmin, dmax = d.min(), d.max()

    # initialize the best threshold and best F1-score found so far
    best_thr = dmin
    best_f1  = 0.0

    # generate 'num_thresholds' equally spaced threshold values between dmin and dmax
    thresholds = np.linspace(dmin, dmax, num_thresholds)

    # initialize a list to store the F1-scores computed at each threshold
    f1_scores = []
    ...
    # visualize the F1-score curve and/or save the figure
    if visualize or save_path:
        plt.figure(figsize=(8, 5))
        plt.plot(thresholds, f1_scores, label="F1 Score")
        plt.axvline(best_thr, color='r', linestyle='--', label=f'Best Threshold = {best_thr:.4f}')
        ...
        if save_path:
            ...
            print(f"📁 F1-score vs Threshold plot saved to: {save_path}")
        if visualize:
            plt.show()
        plt.close()
...
for dataset_name in datasets.keys():
    ...
    best_threshold, best_f1 = compute_threshold_f1(
        distances, binary_labels, visualize=True, save_path=f"{dataset_name}_f1_threshold_plot.png"
    )
